...
```json
{
  "primaryTaskId": "VA_M-001",
  "parallelTracks": ["VA_M-002", "VA_M-003"],
  "dependencyGraph": {
    "VA_M-001": [],
    "VA_M-002": [],
    "VA_M-003": ["VA_M-002"]
...
```bash
node scripts/sprint-board.mjs update --id VA_M-001 --state "In Progress"
node scripts/sprint-board.mjs journal --task VA_M-001 --summary "what changed"
```
...
4. When marking a task `Failed`, record a pitfall entry alongside the state update:
   ```bash
   node scripts/sprint-board.mjs update --id VA_M-001 --state "Failed" \
     --failure-type <gate|acceptance|review> --attempted "..." --hypothesis "..."
   node scripts/sprint-board.mjs pitfall --task VA_M-001 \
     --failure-type <gate|acceptance|review> --attempted "..." --hypothesis "..." \
     [--missing-context "..."]
   ```
